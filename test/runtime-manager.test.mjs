...
test('managed runtime links provider session, lease closeout, mission task, and events', async t => {
  ...
  assert.equal(control.snapshot().runs[0].taskPacket.source, 'mission')
  assert.equal(control.snapshot().runs[0].taskPacket.input.chars <= 8000, true)
  assert.equal(control.snapshot().runs[0].taskPacket.input.sha256.length, 64)
...
test('managed runtime marks an overlong task packet as truncated without storing its prompt', async t => {
  const dir = scratchDir(t, 'quorum-runtime-packet-')
  const control = new AgentControlManager({ store: new AgentControlStore(dir) })
  defer(t, () => control.store.close())
  const missions = new MissionStore(path.join(dir, 'missions.json'))
  const mission = missions.create({ title: 'Packet boundary', objective: 'Keep prompts private.', tasks: [{ id: 'task', title: 'Long task' }] })
  const child = fakeChild()
  const manager = new RuntimeManager({ agentControl: control, missions, memoryBridge: { recall: async () => '' }, executablePathImpl: () => true, spawnImpl: async () => child, heartbeatMs: 60_000 })
  const privateMarker = 'private-task-marker'
  const started = await manager.start({ missionId: mission.id, taskId: 'task', runtime: 'codex', role: 'builder', cwd: dir, worktree: dir, task: `${privateMarker}${'x'.repeat(9000)}` })
  const packet = control.getRun(started.run.runId).taskPacket
  assert.equal(packet.input.truncated, true)
  assert.equal(packet.input.chars, 8000)
  assert.equal(JSON.stringify(packet).includes(privateMarker), false)
  manager.cancel(started.run.runId)
  child.emit('exit', 143, 'SIGTERM')
  await new Promise(resolve => setImmediate(resolve))
})

test('managed runtime cancellation sends termination and records cancelled task', async t => {
  const dir = scratchDir(t, 'quorum-runtime-cancel-')
  const control = new AgentControlManager({ store: new AgentControlStore(dir) })
  defer(t, () => control.store.close())
  const missions = new MissionStore(path.join(dir, 'missions.json'))
...
test('provider spawn failure closes the lease instead of orphaning the mission', async t => {
  const dir = scratchDir(t, 'quorum-runtime-spawn-')
  const control = new AgentControlManager({ store: new AgentControlStore(dir) })
  defer(t, () => control.store.close())
  const missions = new MissionStore(path.join(dir, 'missions.json'))
