...
function makeWorkspace(t) {
  const root = scratchDir(t, 'quorum-ws-')
...
test('discovery finds directories with project markers and skips the rest', t => {
  const { root, mk } = makeWorkspace(t)
...
test('discovered projects get readable labels, not raw folder names', t => {
  const { root, mk } = makeWorkspace(t)
...
test('a discovered project wins cwd resolution over the catch-all', t => {
  const { root, mk } = makeWorkspace(t)
