...
function connectionStateFor(error) {
  const code = errorCode(error)
  return code === 'auth-required' || code === 'protocol-error' ? code : 'degraded'
}
...
export class OpenClawBridge {
  constructor({ env = process.env, url = env.QUORUM_OPENCLAW_URL || 'ws://127.0.0.1:18789', fetchImpl = fetch, WebSocketImpl = NodeWebSocket, clock = () => Date.now(), onUpdate = () => {}, onEvent = () => {} } = {}) {
    ...
    this.socket = null; this.pending = new Map(); this.preview = new Map(); this.previewParams = new Map(); this.reconnectTimer = null; this.heartbeatTimer = null; this.connectSent = false; this.started = false; this.backoffMs = 1000
  ...
  async start() { this.started = true; this.heartbeatTimer = setInterval(() => this.heartbeat(), 15_000); this.heartbeatTimer.unref?.(); await this.probe(); if (this.state.credentials.token.configured || this.state.credentials.password.configured) this.connect(); return this.snapshot() }
  ...
  connect() {
    ...
    socket.on('error', error => { this.patch({ connectionState: connectionStateFor(error), lastError: errorCode(error) }) })
  ...
  disconnect() { this.started = false; clearTimeout(this.reconnectTimer); clearInterval(this.heartbeatTimer); this.reconnectTimer = null; this.heartbeatTimer = null; this.socket?.close?.(); this.socket = null; this.patch({ connectionState: 'offline', authState: 'unknown' }) }

  heartbeat() {
    if (!this.socket || this.state.connectionState !== 'connected') return
    const last = Math.max(this.state.lastEventAt || 0, this.state.lastHandshakeAt || 0)
    if (last && this.clock() - last > 45_000) { this.patch({ connectionState: 'degraded', lastError: 'gateway heartbeat stale' }); this.socket.close?.(); return }
    this.socket.ping?.()
  }
  ...
  receive(raw) {
    ...
    if (frame.type === 'res') {
      if (frame.id === 'quorum-connect') {
        ...
        else { const code = errorCode(frame.error); this.patch({ connectionState: connectionStateFor(frame.error), authState: code === 'auth-required' ? 'required' : 'unknown', lastError: code }); this.socket?.close?.() }
        return
      }
  ...
  requestSnapshot() {
    const projections = [['sessions.list', 'sessions'], ['cron.list', 'cronJobs'], ['channels.status', 'channels'], ['nodes.list', 'nodes'], ['skills.list', 'skills'], ['config.get', 'config']]
    for (const [method, key] of projections) this.request(method, {}).then(value => { this.state.projection[key] = publicParams(value); this.patch({ projection: this.state.projection }) }).catch(() => {})
  }
  ...
  cancelAction(previewId) {
    ...
    record.status = 'cancelled'; this.preview.delete(record.id); this.previewParams.delete(record.id); this.audit(record, 'cancelled'); return structuredClone(record)
  }
  ...
  audit(record, outcome) { this.state.audit = [...this.state.audit, { schemaVersion: 1, id: record.id, method: record.method, risk: record.risk, outcome, actor: record.actor, reason: record.reason, rollback: record.rollback || null, ts: this.clock() }].slice(-100); this.patch({ audit: this.state.audit }) }
